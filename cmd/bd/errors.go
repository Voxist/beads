...
// command is `bd migrate --force` / `bd migrate schema --force` (fork). --force
// already means "this process is the designated migrator" to the
// remote-migrate gate; it means the same thing to a freeze, or a frozen fleet
// has no way to be migrated except by thawing it first -- which lets every
// other process race in, which is what the freeze was for.
var migrateForceOverridesFreeze bool

// freezeOverrideWarned keeps the override warning to one line per process: the
// refusal is consulted from the root pre-run AND from the command's own
...
func migrationFreezeRefusal(operation string, res migration.Result) error {
	...
	// Fork: --force lets the designated migrator through a readable freeze. It
	// deliberately does not reach the fail-closed branch above: a marker bd
	// cannot read is not one it can knowingly override.
	if migrateForceOverridesFreeze {
		if !freezeOverrideWarned {
			freezeOverrideWarned = true
			who := ""
			if info := migration.ReadFile(res.Path); info != nil && info.Operator != "" {
				who = " (by " + ui.SanitizeForTerminal(info.Operator) + ")"
			}
			fmt.Fprintf(os.Stderr, "Warning: workspace is frozen for migration%s at %s; continuing because --force names this process the designated migrator.\n", who, res.Path)
		}
		return nil
	}
...
// FatalError writes an error message and exits with code 1. Fork-local hard-exit
// helper for proxied-server paths that cannot thread an error up through RunE
// (upstream removed these in 5d87c3001; the fork's S5 proxied helpers still need
// them). Prefer HandleError in RunE command bodies.
...
// FatalErrorRespectJSON writes an error message and exits with code 1. If --json
// is set, outputs structured JSON to stdout; otherwise plain text to stderr.
