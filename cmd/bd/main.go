...
var rootCmd = &cobra.Command{
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) (retErr error) {
		...
		// Fork: --force also names this process the designated migrator under a
		// MIGRATION-FREEZE, so this is resolved BEFORE the freeze lookup below.
		// The one process allowed through a freeze is the one there to migrate.
		...
		//
		// Fork: the same answer is handed to the schema layer. The CLI gate
		// refuses writes, but a read's store open still runs schema.MigrateUp,
		// and without SetMigrationFrozen a `bd list` under an active freeze
		// migrated the database anyway (measured: cursor 60 -> 66 with the
		// marker present).
		schema.SetMigrationFrozen(commandFreeze.Frozen())
