...
// (dc-6jaq).
//
// In this fork the freeze also stops schema migration on read commands: cmd/bd
// hands the marker's state to the schema layer (schema.SetMigrationFrozen) and
// MigrateUp refuses pending work under it on every backend. The one override is
// `bd migrate --force`, which names the designated migrator and does not remove
// the marker.
package migration
