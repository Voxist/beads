...
func applyTypeSuppressions(in issueops.ListRequest, cfg ListConfig, filter *types.IssueFilter) {
	...
	// type exclusions.
	//
	// FORK DELTA (va-k0e/vg-3kn, engdocs/FORK_DIVERGENCE.md): ANY explicit type
	// admits the plane, not only an infra one. The write path routes on storage
	// class, not type — internal/storage/dolt/issues.go's useWispsTable is
	// `Ephemeral || NoHistory || WispType != "" || IsInfraType`, so a no_history
	// task or molecule sits in the wisps TABLE with ephemeral = 0. Under the
	// upstream condition `bd list --type task` skips that table and those beads
	// are invisible with no indication anything was withheld. Naming a type is
	// a request for that type wherever it lives, so it opts the plane back in.
	// This makes the infra clause redundant: an infra type is a non-empty one.
