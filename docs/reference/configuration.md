...
For most developers no configuration is needed — issue authorship matches commit authorship automatically.

Under gc/gastown orchestration, `GC_AGENT` is normally already set in the environment and needs no configuration either: it lets `bd update --claim` resolve to the same identity a formula used for an earlier `--assignee=$GC_AGENT`, so re-claiming an already-self-assigned issue is idempotent instead of failing with "already claimed" against a different (git/$USER) identity.
