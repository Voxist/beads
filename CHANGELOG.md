...
- **Fork: the migration freeze also stops schema migration on reads, and
  `bd migrate --force` is its one override** (dc-6jaq). Upstream's freeze
  (entry above) skips bd's own maintenance on a read; in this fork the marker's
  state also reaches `schema.MigrateUp`, which refuses pending work on every
  backend, so a read's store open cannot migrate a frozen store either
  (measured before the gate: cursor 60 -> 66 under a `bd list`). `bd migrate
  --force` names the designated migrator and passes a readable freeze on both
